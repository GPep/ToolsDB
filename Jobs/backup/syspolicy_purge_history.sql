...
IF EXISTS (SELECT name FROM msdb.dbo.sysjobs WHERE name = 'DBA: syspolicy_purge_history')
BEGIN
EXEC msdb.dbo.sp_delete_job @job_name=N'DBA: syspolicy_purge_history', @delete_unused_schedule=1
END

DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA: syspolicy_purge_history', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'purge history', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'C4rb0n', 
		@notify_email_operator_name=N'Database Administrators', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Verify that automation is enabled.]    Script Date: 24/09/2018 07:55:44 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Verify that automation is enabled.', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Purge history.]    Script Date: 24/09/2018 07:55:45 ******/
