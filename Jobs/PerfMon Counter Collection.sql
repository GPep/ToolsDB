...
IF EXISTS (SELECT name FROM msdb.dbo.sysjobs WHERE name = 'DBA: PerfMon Counter Collection')
BEGIN
EXEC msdb.dbo.sp_delete_job @job_name=N'DBA: PerfMon Counter Collection', @delete_unused_schedule=1
END
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Get PerfMon data]    Script Date: 24/09/2018 09:26:40 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Get PerfMon data', 
		...
		@command=N'EXEC GetPerfCountersFromPowerShell', 
		@database_name=N'Tools', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ClearPerfCtrHistory]    Script Date: 24/09/2018 09:26:40 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'ClearPerfCtrHistory', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXECUTE ClearPerfCtrHistory', 
		@database_name=N'Tools', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
